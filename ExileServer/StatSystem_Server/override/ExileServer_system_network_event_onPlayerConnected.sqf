...
private["_isKnownAccount", "_isKnownStatAccount"];
_uid = _this select 0;
...
if !(_uid in ["", "__SERVER__", "__HEADLESS__"]) then
{
	...
	_isKnownStatAccount = format["hasStatAccount:%1", _uid] call ExileServer_system_database_query_selectSingleField;

	if (_isKnownAccount) then
	{
		format["startAccountSession:%1:%2", _uid, _name] call ExileServer_system_database_query_fireAndForget;

		if (not _isKnownStatAccount) then {
			format["createStatAccount:%1", _uid] call ExileServer_system_database_query_fireAndForget;
		};
	}
	else 
	{
